...
def calcAreas(path, name):
    ...
    print(d.keys())
    background = 255 - (d[get_val_by_psevdo(d,'vesicles')] + d[get_val_by_psevdo(d, 'mitochondria')] + d[get_val_by_psevdo(d, 'boundaries')])
    ...
    return vesicles, mitochondria, boundaries, ground
...
def calcSlice(path, name):
    ...
    return bin_edges, vesicles, mitochondria, boundaries, ground

def printPlot(title, bin_edges, vesicles, mitochondria, boundaries, ground, view_data=False, save_path=None):
...
def run_check_statistic_syn_dataset(syn_dataset_path, view_data=False):
    ...
    for f in go:
        f = f.split('\\')[-1]
        bin_edges, vesicles, mitochondria, boundaries, ground = calcSlice(etal_path, f)
    ...
    #printPlot('Original all layers',bin_edges, o_vesicles, o_mitochondria, o_boundaries, o_ground)
    bin_edges2, vesicles2, mitochondria2, boundaries2, ground2 = calcSlice(etal_path, "training0000.png")
    #printPlot('Original 0 layer',bin_edges2, vesicles2, mitochondria2, boundaries2, ground2)
    ...
    for f in g:
        f = f.split('\\')[-1]
        bin_edges, vesicles, mitochondria, boundaries, ground = calcSlice(syn_dataset_path, f)
        sumvesicles = sumvesicles + vesicles
        summitochondria = summitochondria + mitochondria
        sumboundaries = sumboundaries + boundaries
        sumground = sumground + ground
    ...
    printPlot('Synthetic', bin_edges, sumvesicles, summitochondria, sumboundaries, sumground, view_data=view_data, save_path=syn_dataset_path)
...
if __name__ == "__main__":
    syn_dataset_path = r"D:/Projects/Synthetics/Synthetic3D/datasets/dataset_2026_06_24__19_23_56"
