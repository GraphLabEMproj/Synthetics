...
_GROUP_ORDER  = ['main','axon','membrane','mitohondrion','vesicles','poisson','other']
_GROUP_LABELS = {
    'main':'Фон', 'axon':'Аксон', 'membrane':'Мембраны',
    'mitohondrion':'Митохондрии',
    'vesicles':'Везикулы',
    'poisson':'Шум', 'other':'Прочее',
}

def _get_group(key: str) -> str:
    k = key.lower()
    for prefix in ['mitohondrion','membrane','vesicles','axon','main']:
...
def _fmt_label(key: str) -> str:
    label = key
    for p in ['main_','axon_','membrane_','mitohondrion_', 'vesicles_']:
...
MASK_PROVIDER_KEYS  = ['layer','mask_axon','mask_mem','mask_mito', 'mask_ves']
MASK_DISPLAY_LABELS = ['Оригинал','Аксон','Мембраны','Митохондрии','Везикулы']


# ── image provider ─────────────────────────────────────────────────────────────

class Synthetics3DImageProvider(QQuickImageProvider):
    def __init__(self):
...
class GenerationWorker(QThread):
    ...
    def _run_inner(self):
        ...
        try:
            ...
            # Удалить прошлую log сессию
            clear_log_file()
  
            # Прочитать имеющийся конфиг
            ...
            if self._mode == 'single':
                ...
                log(f"▶ Отрисовка {sz}×{sy}×{sx} vx...")
                generator.CreateDataBackGround()
                generator.DrawDataset()
                generator.AddBlur()
                generator.AddNoise()
                generator.DrawMasks()
                results = [generator.data] + generator.masks # получается список 3D тензоров
                log(f"✓ Готово {sz}×{sy}×{sx} vx")
...
class Backend(QObject):
    ...
    def __init__(self, provider: Synthetics3DImageProvider, parent=None):
    ...
    def _on_done(self, layer, mask_axon, mask_mem, mask_mito, mask_ves):
        for i in range(len(layer)):
            self._last_layer = layer[i]
            self._counter   += 1
            def _g(m):   # 3-ch mask → 2D grayscale
                return np.ascontiguousarray(m[i, :, :, 0]) if m is not None and m.ndim == 4 else m[i]
